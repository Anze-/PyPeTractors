...
#define img_d 1 //image density, 1 means 1 pixel for unit of the D (wich is the radius of the sphere of the pendulum)
...
int main(int argc, char **argv)
{
    ...
    #pragma omp parallel shared(mat,chunk,a,b,c) private(i_row,i_col) num_threads(8)
    {
    	...
    	for(i_row = 0; i_row < img_s; i_row++)
		{
			for(i_col = 0; i_col < img_s / 2; i_col++)
			{
			  ...
			  {
					  integrate_n_steps(controlled_runge_kutta< runge_kutta_dopri5< state_type > >(), tractors , x , 0.0 , dt , 2500 , asint_checker);
					  if(x[1] < a.x + tol && x[1] > a.x - tol && x[3] < a.y + tol&& x[3] > a.y - tol)
					  {
						   mat[i_row][i_col] = 'r';
						   mat[i_row][img_s - i_col - 1] = 'g';
